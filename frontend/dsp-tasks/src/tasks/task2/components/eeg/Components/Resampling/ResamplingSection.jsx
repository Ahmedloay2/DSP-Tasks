...
/**
 * ResamplingSection Component for EEG
 * Allows user to resample EEG signal and view aliasing effects
...
const ResamplingSection = ({ recordName, apiMode = 'mock' }) => {
  ...
  const handleResample = async () => {
    ...
    try {
      if (apiMode === 'real') {
        // Real API call
        const response = await fetch(`/api/resample/eeg/${recordName}`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ targetFrequency: resamplingFreq })
        });
        ...
        setResampledData(data);
      } else {
        // Mock API call - simulate network delay
        await new Promise(resolve => setTimeout(resolve, 1500));

        // Use centralized mock service for consistency
        const mockData = MockEEGService.generateSampledSignal(resamplingFreq, 10);
        setResampledData(mockData);
      }
    ...
    <div className="task2-resampling-section-container">
      <div className="task2-resampling-header">
        ...
        <p className="task2-resampling-description">
          Adjust the sampling frequency to observe aliasing effects on the EEG signal
        </p>
      </div>
            ...
            {resamplingFreq < 200 && (
              <div className="task2-aliasing-warning">
                ...
                <div className="task2-warning-content">
                  ...
                  <p>Sampling frequency ({resamplingFreq} Hz) is below the Nyquist rate for typical EEG signals. Aliasing effects may distort the signal representation.</p>
                </div>
              </div>
            )}
            {resamplingFreq >= 200 && resamplingFreq < 500 && (
