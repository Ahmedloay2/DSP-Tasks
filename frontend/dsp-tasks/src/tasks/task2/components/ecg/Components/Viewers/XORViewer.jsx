...
const XORViewer = ({
  ...
  const [hasReachedEnd, setHasReachedEnd] = useState(false); // New state to track end condition
  ...
  useEffect(() => {
    ...
    if (chunk1Start >= channelData.length) {
      // Only set hasReachedEnd to true, don't reset the data
      if (!hasReachedEnd) {
        setHasReachedEnd(true);
      }
      return; // Keep the existing data
    }
    
    // Reset end flag when we have new data
    if (hasReachedEnd) {
      setHasReachedEnd(false);
    }
    ...
    // Phase 2: Keep full chunk 1, stream chunk 2 sample by sample
      ...
      if (chunk2Start < channelData.length) {
        ...
        // We've reached the end for chunk2, but don't reset - keep what we have
        // Only set to null if we don't have any data yet
        if (!chunk2Data) {
          setChunk2Data(null);
          setXorResult(null);
        }
  ...
  }, [channels, selectedChannel, currentTime, chunkSize, samplingRate, hasReachedEnd, chunk2Data]);

  // Draw chunk info overlay - UPDATED to show end state
  const drawChunkInfo = useCallback((ctx) => {
    if (!selectedChannel) return;

    // Adjust height based on whether we've reached the end
    const infoBoxHeight = hasReachedEnd ? 90 : 70;
    
    // Draw simple info box
    ctx.fillStyle = 'rgba(255, 255, 255, 0.95)';
    ctx.fillRect(5, 5, 280, infoBoxHeight);
    ctx.strokeStyle = '#2c3e50';
    ctx.lineWidth = 2;
    ctx.strokeRect(5, 5, 280, infoBoxHeight);
    ...
    // Add end of data indicator
    if (hasReachedEnd) {
      ctx.fillStyle = '#e74c3c';
      ctx.fillText('End of data reached', 15, 85);
    }
  }, [selectedChannel, chunkSize, currentChunkIndex, hasReachedEnd]);
  ...
  return (
    <div className="task2-xor-viewer">
      <div className="task2-viewer-header">
        <h3 className="task2-viewer-title">XOR Graph View</h3>
        <div className="task2-chunk-size-control">
          <label className="task2-chunk-label">Chunk Size:</label>
          <input
            type="range"
            min={1}
            max={10}
            ...
            className="task2-chunk-slider"
          />
          <span className="task2-chunk-value">{chunkSize}s</span>
        </div>
      </div>

      <div className="task2-xor-content">
        <div className="task2-xor-right">
          <canvas
            ref={canvasRef}
            className="task2-xor-canvas"
          />

          <div className="task2-xor-info">
            <div className="task2-selected-channel-info">
              {selectedChannel ? (
                <>
                  <div
                    className="task2-channel-indicator"
                    style={{ backgroundColor: CHANNEL_CONFIG[selectedChannel].color }}
                  />
                  <span className="task2-channel-text">
                    <strong>Selected:</strong> {CHANNEL_CONFIG[selectedChannel].name}
                  </span>
                </>
              ) : (
                <span className="task2-no-channel-text">← Select 1 channel to view XOR</span>
              )}
            </div>
            
            {hasReachedEnd && (
              <div className="task2-end-indicator">
                <strong>End of data reached - Displaying final chunks</strong>
              </div>
            )}
            
            {chunk1Data && chunk2Data && (
              <div className="task2-xor-statistics">
                <p className="task2-stat-item">
