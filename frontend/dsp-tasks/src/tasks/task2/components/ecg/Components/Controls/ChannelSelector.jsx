...
const ChannelSelector = ({ 
  ...
  return (
    <div className="task2-channel-selector">
      <div className="task2-channel-selector-header">
        <h3 className="task2-channel-selector-title">
          Channel Selection
          <span className="task2-channel-count">
            ({selectedCount}/{maxSelection || 12})
          </span>
        </h3>
        {!maxSelection && (
          <div className="task2-channel-selector-actions">
            <button
              className="task2-channel-action-btn"
        ...
        {maxSelection && (
          <div className="task2-selection-hint">
            {maxSelection === 1 && <span className="task2-hint-text">Select 1 channel</span>}
            {maxSelection === 2 && <span className="task2-hint-text">Select 2 channels (X → Y)</span>}
          </div>
        )}
      </div>

      <div className="task2-channel-grid">
        {CHANNEL_NAMES.map(channelId => {
          ...
          return (
            <div
              key={channelId}
              className={`task2-channel-item ${isSelected ? 'selected' : ''} ${!canSelect ? 'disabled' : ''}`}
              onClick={() => handleChannelClick(channelId)}
              style={{ cursor: canSelect ? 'pointer' : 'not-allowed' }}
            >
              <div
                className="task2-channel-color-indicator"
                style={{ backgroundColor: config.color }}
              />
              <span className="task2-channel-name">{config.name}</span>
              <div className="task2-channel-checkbox">
                {isSelected && <span className="task2-checkmark">✓</span>}
                {isSelected && selectionLabel && (
                  <span className="task2-selection-label">{selectionLabel}</span>
                )}
              </div>
            </div>
          );
