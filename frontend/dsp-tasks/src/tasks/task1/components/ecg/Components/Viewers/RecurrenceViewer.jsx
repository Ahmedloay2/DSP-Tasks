...
const RecurrenceViewer = ({
  ...
  useEffect(() => {
    ...
    const points = RealECGDataService.createRecurrencePlot(dataX, dataY, 0, maxIndex);
    ...
    if (points.length > 100) {
      try {
        const density = RealECGDataService.calculateDensityMap(points, 50);
  ...
  // All control handlers are now in parent (MultiChannelECGViewer)
