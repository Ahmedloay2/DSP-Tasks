...
const PolarViewer = ({
    ...
    if (polarMode === POLAR_MODES.CUMULATIVE) {
      ...
      selectedChannelIds.forEach(channelId => {
        ...
        if (channelData) {
          ...
          const polarCoords = RealEEGDataService.toPolarCoordinates(channelData, 0, endIndex);
  ...
  const drawLatestFixed = (ctx, centerX, centerY, maxRadius) => {
    ...
    selectedChannelIds.forEach(channelId => {
      ...
      const polarCoords = RealEEGDataService.toPolarCoordinates(channelData, startIndex, sampleCount);
