...
import { CHANNEL_CONFIG, getChannelColor, getDynamicChannelConfig } from '../../constants/MultiChannelConfig';
import './ContinuousViewer.css';

/**
 * ContinuousViewer Component
 * Real-time EEG monitor with streaming data and circular buffers
 * Data   return (
    <div className="continuous-viewer" style={{ backgroundColor: '#1a1a1a' }}>
      <div className="viewer-header" style={{ backgroundColor: 'transparent', borderBottom: '1px solid #444' }}>
        <h3 className="viewer-title" style={{ color: '#fff' }}>Real-Time EEG Monitor</h3>lls RIGHT TO LEFT like traditional EEG monitors
 */
const ContinuousViewer = ({
  ...
  const draw = useCallback(() => {
    ...
    selectedChannelIds.forEach((channelId, index) => {
      ...
      // Get unique color for each channel from dynamic or static config
      const channelIndex = channelNames.indexOf(channelId);
      const channelConfig = getDynamicChannelConfig(channelId, channelIndex, channelMetadata?.channelNames);
      const color = channelConfig?.color || getChannelColor(channelId);
  ...
  }, [canvasSize, signalBuffers, selectedChannels, cursorPosition, drawGrid, drawSignalWithBuffer, drawCursor, drawSweepLine, channelNames, channelMetadata]);
  ...
  return (
    <div className="continuous-viewer">
      <div className="viewer-header">
        <h3 className="viewer-title">Real-Time EEG Monitor</h3>
          ...
          <div className="amplitude-grid">
            {Object.keys(selectedChannels)
              ...
              .map((channelId) => {
                ...
                return (
                  <div key={channelId} className="amplitude-item">
                    <div
                      className="amplitude-color"
                      style={{ backgroundColor: channelConfig?.color || getChannelColor(channelId) }}
                    />
